...
def buildDiscriminator():
    ...
    model.add(Conv2D(64, (5, 5), strides=(1, 1), padding='same', input_shape=IMAGE_SHAPE))
...
def buildGenerator():
    ...
    model.add(Reshape((IH, IW, IZ)))
    model.add(Conv2DTranspose(64, kernel_size=5, strides=1, padding="same",
                              activation="relu"))
    model.add(BatchNormalization())

    model.add(Conv2DTranspose(32, kernel_size=5, strides=1, padding="same",
                              activation="relu"))
    model.add(BatchNormalization())

    model.add(Conv2DTranspose(1, kernel_size=5, strides=1, padding='same', use_bias=False, activation="tanh"))
...
def buildGAN(images, epochs=40000, batchSize=32, loggingInterval=0, dRatio=1.0, gRatio=1.0):
    # Setup
    opt = Adam(lr=0.0002)
    ...
    for epoch in range(epochs):
        ...
        randRatio = random.random()
        advLossStore = 0.0
        advLossStoreAccuracy = 0.0
        genLossStore = 0.0
        if dRatio - randRatio >= 0:
            ...
            advLossStore = advLoss[0]
            dLoss.append(advLossStore)
            advLossStoreAccuracy = advLoss[1]
            d2Loss.append(advLossStoreAccuracy)

        if gRatio - randRatio >= 0:
            noise = np.random.normal(0, 1, (batchSize, NOISE_SIZE))
            genLoss = gan.train_on_batch(noise, trueCol)
            genLossStore = genLoss
            gLoss.append(genLossStore)

        # Logging
        if loggingInterval > 0 and epoch % loggingInterval == 0:
            print("\tEpoch %d:" % epoch)
            print("\t\tDiscriminator loss: %f." % advLossStore)
            print("\t\tDiscriminator accuracy: %.2f%%." % (100 * advLossStoreAccuracy))
            print("\t\tGenerator loss: %f." % genLossStore)
            runGAN(generator, OUTPUT_DIR + "/" + OUTPUT_NAME + "_test_%d.png" % (epoch / loggingInterval))

    plt.figure(1)
    plt.plot(range(len(dLoss)), dLoss, color='black', label='false loss')
    plt.plot(range(len(d2Loss)), d2Loss, color='green', label='true Loss')
    plt.xlabel("Number of Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Loss plot: discriminator")
    plt.savefig(OUTPUT_NAME + "_dLossPlot.pdf")

    plt.figure(2)
    plt.plot(range(len(gLoss)), gLoss, color='red')
    plt.xlabel("Number of Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Loss plot: generator")
    plt.savefig(OUTPUT_NAME + "_GLossPlot.pdf")
...
def main():
    ...
    (generator, adv, gan) = buildGAN(data, epochs=60000, loggingInterval=1000, dRatio=0.2, gRatio=0.8)
